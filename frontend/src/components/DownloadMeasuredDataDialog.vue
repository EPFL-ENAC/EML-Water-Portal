    ...
    <q-card :style="$q.screen.lt.md ? '' : 'width: 600px; max-width: 80vw'">
      ...
      <q-card-section>
        <div class="text-h6 q-mb-sm">
          {{ t('download.measured_data.title') }}
        </div>
        <div class="q-mb-md text-help">
          {{ t('download.measured_data.content') }}
        </div>
        <q-option-group
          v-model="group"
      ...
      <q-card-actions v-if="$q.screen.gt.xs" align="right">
        ...
        <q-btn
          ...
          color="primary"
          :disable="downloading || measuresVisible.length === 0"
          @click="onDownload"
        />
      </q-card-actions>
    </q-card>
...
const groupOptions = computed(() => [
  { label: t('download.measured_data.group.parameters'), value: 'parameters' },
  { label: t('download.measured_data.group.sources'), value: 'sources' },
]);
