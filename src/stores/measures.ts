...
export const useMeasuresStore = defineStore('measures', () => {
  ...
  function populateSensorData(file: string, header: string[], rows: string[][]) {
    datasets.value?.sensors.filter((sensor) => sensor.file === file).forEach((sensor) => {
      sensor.columns.forEach((col) => {
        const idx = header.indexOf(col.name);
        if (idx < 0) {
          console.error(`Column ${col.name} not found in ${file}`);
        } else {
          const vector = rows.map((row) => row[idx]).map((val: string) => {
            if (col.measure === 'timestamp') {
              // parse with date-fns if different from target format
              const sanitized = val.replaceAll('\n', '').trim();
              try {
                return col.format && col.format !== DATETIME_FORMAT ? format(parse(sanitized, col.format, new Date()), DATETIME_FORMAT) : sanitized;
              } catch(err) {
                console.error(err);
                return sanitized;
              }
              ...
              try {
                ...
                const sanitized = val.replaceAll(',', '.').replaceAll('\n', '').replaceAll(' ', '');
                // Parse the float
